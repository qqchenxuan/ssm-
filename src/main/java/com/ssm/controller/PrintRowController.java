...
@Controller
public class PrintRowController {
	@Autowired
	private MainModelService mainModelService;
	
	@RequestMapping("/print")
	public String print(Model model,String para){
        ...
        for (MainModel mainModel : list) {
        	...
        	row.createCell((short) 1).setCellValue( mainModel.getBom_descCH());
        	row.createCell((short) 2).setCellValue( mainModel.getBom_PN());
        	row.createCell((short) 3).setCellValue((int) 1);
		}
